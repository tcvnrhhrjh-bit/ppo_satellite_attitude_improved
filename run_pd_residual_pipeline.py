...
def write_report(path, rows, train_dir, benchmark_dir, model_path, scenario, args):
    ...
    lines = [
        ...
        f"- `pd_torque_limit`: {args.pd_torque_limit}",
        f"- `combined_torque_limit`: {args.combined_torque_limit}",
        f"- `residual_scale`: {args.residual_scale}",
        f"- `residual_gate_mode`: {args.residual_gate_mode}",
        f"- `residual_gate_min_error_deg`: {args.residual_gate_min_error_deg}",
        f"- `residual_gate_alignment`: {args.residual_gate_alignment}",
        ...
        f"- `disturbance_level`: {args.disturbance_level}",
        f"- `actuator_deadzone`: {args.actuator_deadzone}",
        f"- `coulomb_friction`: {args.coulomb_friction}",
        f"- `viscous_friction`: {args.viscous_friction}",
        f"- `actuator_efficiency`: {args.actuator_efficiency}",
        ...
        f"- `telemetry_min_initial_error`: {args.telemetry_min_initial_error}",
        f"- `telemetry_max_initial_error`: {args.telemetry_max_initial_error}",
        f"- `use_housekeeping`: {args.use_housekeeping}",
        f"- `housekeeping_dir`: {args.housekeeping_dir}",
        f"- `housekeeping_wheel_momentum_limit`: {args.housekeeping_wheel_momentum_limit}",
        f"- `housekeeping_wheel_speed_reference`: {args.housekeeping_wheel_speed_reference}",
        f"- `housekeeping_disturbance_scale`: {args.housekeeping_disturbance_scale}",
        "",
        "## Verdict",
        "",
        verdict,
        ...
        "## Key Metrics",
        "",
        "| Controller | Success rate | Mean final error (deg) | Mean control energy | Applied energy | Command torque | Applied torque | Applied / command |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in sorted(rows, key=lambda r: r["controller"]):
        lines.append(
            ...
            f"{as_float(row, 'mean_applied_control_energy'):.6f} | "
            f"{as_float(row, 'mean_command_torque_norm'):.6f} | "
            f"{as_float(row, 'mean_applied_torque_norm'):.6f} | "
            f"{as_float(row, 'mean_command_to_applied_ratio'):.3f} |"
        )
    ...
    lines.extend([
        ...
        "The command/applied torque columns show whether different controller commands survive actuator dead zones, friction, and reaction-wheel saturation.",
        "",
        "## Output Files",
        "",
...
def main():
    ...
    if not args.skip_training:
        train_command = [
            ...
            "--residual-gate-mode", args.residual_gate_mode,
            "--residual-gate-min-error-deg", str(args.residual_gate_min_error_deg),
            "--residual-gate-alignment", str(args.residual_gate_alignment),
        ...
        if args.combined_torque_limit is not None:
            train_command.extend(["--combined-torque-limit", str(args.combined_torque_limit)])
        for option_name, option_value in [
            ("--disturbance-level", args.disturbance_level),
            ("--actuator-deadzone", args.actuator_deadzone),
            ("--coulomb-friction", args.coulomb_friction),
            ("--viscous-friction", args.viscous_friction),
            ("--actuator-efficiency", args.actuator_efficiency),
        ]:
            if option_value is not None:
                train_command.extend([option_name, str(option_value)])
        ...
        if args.use_telemetry_reset:
            ...
            if args.telemetry_min_initial_error is not None:
                train_command.extend(["--telemetry-min-initial-error", str(args.telemetry_min_initial_error)])
            if args.telemetry_max_initial_error is not None:
                train_command.extend(["--telemetry-max-initial-error", str(args.telemetry_max_initial_error)])
        if args.use_housekeeping:
            train_command.extend([
                "--use-housekeeping",
                "--housekeeping-dir", args.housekeeping_dir or args.telemetry_dir,
                "--housekeeping-wheel-momentum-limit", str(args.housekeeping_wheel_momentum_limit),
                "--housekeeping-wheel-speed-reference", str(args.housekeeping_wheel_speed_reference),
                "--housekeeping-disturbance-scale", str(args.housekeeping_disturbance_scale),
            ])
    ...
    if args.combined_torque_limit is not None:
        benchmark_command.extend(["--combined-torque-limit", str(args.combined_torque_limit)])
    for option_name, option_value in [
        ("--disturbance-level", args.disturbance_level),
        ("--actuator-deadzone", args.actuator_deadzone),
        ("--coulomb-friction", args.coulomb_friction),
        ...
        if option_value is not None:
            benchmark_command.extend([option_name, str(option_value)])
    if args.use_telemetry_reset:
        ...
        if args.telemetry_min_initial_error is not None:
            benchmark_command.extend(["--telemetry-min-initial-error", str(args.telemetry_min_initial_error)])
        if args.telemetry_max_initial_error is not None:
            benchmark_command.extend(["--telemetry-max-initial-error", str(args.telemetry_max_initial_error)])
    if args.use_housekeeping:
        benchmark_command.extend([
            "--use-housekeeping",
            "--housekeeping-dir", args.housekeeping_dir or args.telemetry_dir,
