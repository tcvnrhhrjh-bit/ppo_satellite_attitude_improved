...
def save_episode_csv(info, filename="episode_history.csv"):
    ...
    with open(filename, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "applied_tx", "applied_ty", "applied_tz",
        ])
        for i in range(len(t)):
            action = actions[i - 1] if 0 < i <= len(actions) else [0.0, 0.0, 0.0]
            effective_action = effective_actions[i - 1] if 0 < i <= len(effective_actions) else [0.0, 0.0, 0.0]
            reward = rewards[i - 1] if 0 < i <= len(rewards) else 0.0
            writer.writerow([
                t[i], *x[i], phi_deg[i], omega_norm[i], reward, *action, *effective_action,
            ])
