...
from baseline_pd import pd_action, run_pd_baseline
from main import make_demo_start_state, parse_angle_list, set_global_seed
from residual_benchmark import DEFAULT_BENCHMARK_ENV, nearest_action, run_trial
...
def residual_env_kwargs(args, max_steps):
    kwargs = dict(DEFAULT_BENCHMARK_ENV)
    kwargs.update({
        "max_steps": max_steps,
...
def combined_residual_torque(env, observation, action_index, residual_scale, pd_kp, pd_kd, torque_limit):
    ...
    return nearest_action(env.action_space, u_total)
