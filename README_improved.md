...
8. Reward shaping was strengthened for convergence.
   ...
   - Degree-based progress shaping now rewards each reduction in attitude error more directly.
   - Regression and stagnation penalties discourage the policy from letting error increase or plateau near the target.
   - Near-target bonuses keep pressure on the policy below 20, 10, and 5 degrees instead of treating "good enough" as finished too early.
...
- `run_pd_residual_pipeline.py`: one-command pipeline that trains PD+PPO residual, benchmarks it against pure PD, and writes a Markdown comparison report.
- `telemetry_loader.py`: loads RIoT-2 quaternion and gyroscope telemetry, normalizes attitude states, and builds reset states for telemetry-driven training and benchmarking.
...
The residual-control reward exposes extra shaping terms:

```bash
python residual_train.py --epochs 300 --progress-weight 8 --near-target-weight 0.2 --fine-pointing-weight 0.6 --regression-weight 4.0 --stagnation-weight 0.04
```
...
Or run the full automated comparison pipeline:

```bash
python run_pd_residual_pipeline.py --scenario residual_friendly --epochs 300 --steps-per-epoch 1000 --train-max-steps 120 --steps 500 --trials 30
```

To reduce steady-state pointing error under fixed disturbance torque, enable the integral correction term:

```bash
python run_pd_residual_pipeline.py --scenario residual_friendly --epochs 300 --steps-per-epoch 1000 --train-max-steps 220 --steps 1000 --trials 30 --residual-scale 0.2 --pd-kp 0.08 --pd-kd 0.80 --pd-ki 0.02 --integral-limit 0.2 --train-angles 10,15,20,25,30 --benchmark-angles 10,15,20,25,30
```

The `pd_ki` term integrates the attitude-error vector and is intended to reduce steady-state error caused by persistent disturbance torque. `integral_limit` clamps the accumulated error to avoid integral windup.

For stronger reward pressure toward lower attitude error, use:

```bash
python run_pd_residual_pipeline.py --scenario residual_friendly --epochs 300 --steps-per-epoch 1000 --train-max-steps 220 --steps 1000 --trials 30 --residual-scale 0.2 --pd-kp 0.08 --pd-kd 0.80 --pd-ki 0.02 --integral-limit 0.2 --progress-weight 8 --near-target-weight 0.2 --fine-pointing-weight 0.6 --regression-weight 4.0 --stagnation-weight 0.04 --train-angles 10,15,20,25,30 --benchmark-angles 10,15,20,25,30
```

To advance to the next curriculum angle only after the current angle has reached a target mean return, enable adaptive curriculum:

```bash
python run_pd_residual_pipeline.py --scenario residual_friendly --adaptive-curriculum --advance-return-threshold -150 --advance-window 5 --min-epochs-per-angle 8 --max-epochs-per-angle 50 --epochs 300 --steps-per-epoch 1000 --train-max-steps 220 --steps 1000 --trials 30 --residual-scale 0.2 --pd-kp 0.08 --pd-kd 0.80 --pd-ki 0.02 --integral-limit 0.2 --progress-weight 8 --near-target-weight 0.2 --fine-pointing-weight 0.6 --regression-weight 4.0 --stagnation-weight 0.04 --train-angles 10,15,20,25,30 --benchmark-angles 10,15,20,25,30
```

Adaptive curriculum uses the rolling mean return from the current angle. If the rolling mean is above `advance-return-threshold` for the selected `advance-window`, the next angle starts. `max-epochs-per-angle` prevents training from getting stuck forever on one angle.

To train and benchmark from reconstructed RIoT-2 telemetry states, put these files in a folder such as `data/riot2_2026_07_07`:

```text
ADCS_Estimated_Quaternion_scaled_2026-07-07.csv
analysis_scaled_8-panels_2026-07-07.csv
```

Then run:

```bash
python run_pd_residual_pipeline.py --scenario residual_friendly --use-telemetry-reset --telemetry-dir data/riot2_2026_07_07 --telemetry-gyro-unit deg/s --telemetry-scalar-component auto --adaptive-curriculum --advance-return-threshold -150 --advance-window 5 --min-epochs-per-angle 8 --max-epochs-per-angle 50 --epochs 300 --steps-per-epoch 1000 --train-max-steps 220 --steps 1000 --trials 30 --residual-scale 0.2 --pd-kp 0.08 --pd-kd 0.80 --pd-ki 0.02 --integral-limit 0.2 --progress-weight 8 --near-target-weight 0.2 --fine-pointing-weight 0.6 --regression-weight 4.0 --stagnation-weight 0.04 --train-angles 10,15,20,25,30 --benchmark-angles 10,15,20,25,30
```

With telemetry reset enabled, `train-angles` still remains available as a fallback and for curriculum metadata, but the actual reset states come from the measured quaternion and gyro rows. The loader automatically normalizes the quaternion and infers which quaternion component is the scalar term.

For a quick pipeline smoke test:

```bash
python run_pd_residual_pipeline.py --output-dir verify_auto_pipeline --scenario residual_friendly --epochs 1 --steps-per-epoch 80 --train-max-steps 40 --steps 60 --train-angles 10 --benchmark-angles 10,20 --trials 2
```

The automated pipeline writes:

```text
automated_comparison_report.md
01_residual_training/
02_benchmark/
```
...
The benchmark supports three repeatable environment presets:

```text
nominal_nonlinear   baseline nonlinear actuator case
residual_friendly   fixed disturbance bias with stronger dead zone and friction
random_disturbance  randomized disturbance torque for robustness testing
```

The `residual_friendly` scenario is useful for demonstrating why PPO can be helpful as a residual controller. Pure PD has no memory or learned feed-forward compensation, while PD+PPO can learn a small correction for persistent disturbance, dead zone, friction, and actuator-efficiency loss.
