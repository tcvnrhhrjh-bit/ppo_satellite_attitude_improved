...
REACTION_WHEEL_MODELS = {
    "none": {
        "torque_limit": None,
        "momentum_limit": None,
        "max_power_w": None,
        "label": "unconstrained generic torque actuator",
    },
    "rw-0.01": {
        "torque_limit": 0.001,
        "momentum_limit": 0.018,
        "max_power_w": 1.05,
        "label": "Rocket Lab/Sinclair RW-0.01",
    },
    "rw-0.03": {
        "torque_limit": 0.002,
        "momentum_limit": 0.040,
        "max_power_w": 1.0,
        "label": "Rocket Lab/Sinclair RW-0.03",
    },
}
...
class TorqueDynamics(gym.Env):
    ...
    def __init__(
        ...
        wheel_momentum_limit=None,
        wheel_power_limit=None,
        seed=None,
    ):
        self.state = None
        self.dt = dt
        ...
        self.near_target_weight = near_target_weight
        self.fine_pointing_weight = fine_pointing_weight
        ...
        if integrator not in {"rk4", "solve_ivp"}:
            raise ValueError("integrator must be 'rk4' or 'solve_ivp'")
        self.integrator = integrator
        self.wheel_model = wheel_model.lower()
        if self.wheel_model not in REACTION_WHEEL_MODELS:
            valid = ", ".join(sorted(REACTION_WHEEL_MODELS))
            raise ValueError(f"wheel_model must be one of: {valid}")
        wheel_defaults = REACTION_WHEEL_MODELS[self.wheel_model]
        self.wheel_torque_limit = (
            wheel_defaults["torque_limit"] if wheel_torque_limit is None else wheel_torque_limit
        )
        self.wheel_momentum_limit = (
            wheel_defaults["momentum_limit"] if wheel_momentum_limit is None else wheel_momentum_limit
        )
        self.wheel_power_limit = (
            wheel_defaults["max_power_w"] if wheel_power_limit is None else wheel_power_limit
        )
        self.wheel_label = wheel_defaults["label"]
    ...
    def reset(self, state=None):
        ...
        self.action_history = []
        self.effective_action_history = []
        self.wheel_momentum_history = [self.state[7:10].copy()]
        self.wheel_saturation_history = []
        self.wheel_power_history = []
        self.disturbance_history = []
    ...
    def shaped_reward(self, phi, prev_phi, omega, action, success):
        error_deg = np.rad2deg(phi)
        prev_error_deg = np.rad2deg(prev_phi)
        progress_deg = prev_error_deg - error_deg
        omega_norm = np.linalg.norm(omega)
        action_norm = np.linalg.norm(action)
        fuel_use = np.linalg.norm(action, ord=1) * self.dt

        progress_reward = self.progress_weight * np.tanh(progress_deg / 2.0)
        if progress_deg > 0:
            progress_reward += 0.20 * progress_deg
        else:
            progress_reward += self.regression_weight * progress_deg

        near_target_fraction = max(0.0, 20.0 - error_deg) / 20.0
        fine_pointing_fraction = max(0.0, 10.0 - error_deg) / 10.0
        plateau_penalty = 0.0
        if abs(progress_deg) < 0.01 and phi > self.attitude_tolerance:
            plateau_penalty = self.stagnation_weight * min(error_deg, 30.0) / 30.0

        reward = (
            -0.020 * error_deg
            - 0.0008 * error_deg**2
            - self.omega_weight * omega_norm
            - self.action_weight * action_norm
            - self.fuel_weight * fuel_use
            - self.time_weight
            + progress_reward
            + self.near_target_weight * near_target_fraction**2
            + self.fine_pointing_weight * fine_pointing_fraction**2
            - plateau_penalty
        )
        if error_deg < 7.0:
            reward += 0.40
    ...
    def step(self, action):
        ...
        effective_action, wheel_saturated = self.apply_reaction_wheel_limits(effective_action)
        disturbance = self.external_disturbance(self.t[-1])

        if self.integrator == "solve_ivp" and solve_ivp is not None:
            sol = solve_ivp(
        ...
        self.wheel_momentum_history.append(observation[7:10].copy())
        self.wheel_saturation_history.append(wheel_saturated.copy())
        self.wheel_power_history.append(self.estimate_wheel_power(effective_action))
        ...
        info = {
            ...
            "wheel_momentum": self.wheel_momentum_history,
            "wheel_saturation": self.wheel_saturation_history,
            "wheel_power": self.wheel_power_history,
    ...
    def apply_actuator_model(self, command, omega):
        ...
        if self.wheel_torque_limit is not None:
            command = np.clip(command, -self.wheel_torque_limit, self.wheel_torque_limit)
    ...
    def apply_reaction_wheel_limits(self, torque):
        torque = np.asarray(torque, dtype=float).reshape(3,)
        saturated = np.zeros(3, dtype=bool)
        if self.wheel_momentum_limit is None:
            return torque, saturated

        wheel_momentum = np.asarray(self.state[7:10], dtype=float)
        predicted_momentum = wheel_momentum - torque * self.dt
        limited_momentum = np.clip(
            predicted_momentum,
            -self.wheel_momentum_limit,
            self.wheel_momentum_limit,
        )
        saturated = np.abs(predicted_momentum - limited_momentum) > 1e-12
        limited_torque = (wheel_momentum - limited_momentum) / self.dt
        return limited_torque, saturated

    def estimate_wheel_power(self, torque):
        if self.wheel_torque_limit in (None, 0) or self.wheel_power_limit is None:
            return 0.0
        load_fraction = np.clip(np.abs(torque) / self.wheel_torque_limit, 0.0, 1.0)
        return float(np.sum(load_fraction) * self.wheel_power_limit)

    def external_disturbance(self, t):
        telemetry_disturbance = np.asarray(self.telemetry_disturbance, dtype=float).reshape(3,)
        if self.disturbance_level <= 0:
            return telemetry_disturbance
        periodic = 0.35 * self.disturbance_level * np.sin(
            0.07 * t + self.disturbance_phase
        )
        return self.disturbance_bias + periodic + telemetry_disturbance
